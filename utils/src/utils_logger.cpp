...
void Logger::init(const LogConfig &config) {
    ...
    if (m_inited.compare_exchange_strong(expired, true)) {
        ...
        for (int i = 0; i < sizeof(log_level_names)/sizeof(log_level_names[0]); ++i) {
...
bool Logger::check_logfile(std::string &logfile, time_t now) {
    ...
    snprintf(filename + strlen(filename), sizeof(filename) - strlen(filename), "_%04d-%02d-%02d_%d_%02lu", 
             tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday, utils::getpid(), m_current_file_index++);
...
void Logger::log_message(LOG_LEVE level, const char *filename, int line, const char *function, const char *format, ...) {
    ...
    size_t size = 0;
    if (m_enable_color) {
        size += snprintf(buff, sizeof(buff),"%04d-%02d-%02d %02d:%02d:%02d.%-6ld %s%-5s%s %lld",
            tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, static_cast<long>(us.count()),
            log_level_begin[level], log_level_names[level], log_level_end[level], thread_id());
    } else {
        size += snprintf(buff, sizeof(buff),"%04d-%02d-%02d %02d:%02d:%02d.%-6ld %-5s %lld",
            tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec, static_cast<long>(us.count()),
            log_level_names[level], thread_id());
    }
